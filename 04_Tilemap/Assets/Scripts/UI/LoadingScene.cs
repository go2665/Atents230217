...
public class LoadingScene : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        loadingTextCoroutine = LoadingTextProgress();   // 코루틴 정지 시키기 위해 저장해 놓기
    ...
    private void Update()
    {
        if(slider.value < loadRatio)        // slider.value를 loadRatio까지 무조건 증가 시키기
        {
    ...
    IEnumerator LoadScene()
    {
        ...
        while (loadRatio < 1.0f)        // loadRatio가 1 이상이 될때까지 반복
        {
            ...
            //image.color = Color.Lerp(start, end, slider.value);
        ...
        StopCoroutine(loadingTextCoroutine);        // 글자 변경하는 코루틴 정지 시키기
